// features/fragments/engine/renderer.ts
// Canvas 2D renderer for fragment drift

import type { DriftingFragment, ConnectionLine } from '../types/fragments.types'

interface DriftRenderState {
  fragments: DriftingFragment[]
  connections: ConnectionLine[]
  selectedFragment: DriftingFragment | null
  width: number
  height: number
  time: number
}
...
export function initDriftRenderer(canvas: HTMLCanvasElement) {
...
export function renderDriftFrame(state: DriftRenderState) {
  ...
  for (const f of state.fragments) {
    drawFragment(ctx, f, state.time, f === state.selectedFragment)
  }
}
...
function drawFragment(
  ...
  switch (f.type) {
    case 'sentence':
      drawStar(ctx, f.x, f.y, 4 * scale, '#ffffff', alpha, time)
      break
    case 'knowledge':
      drawOrb(ctx, f.x, f.y, 6 * scale, '#fcd34d', alpha, time)
      break
    case 'vector':
      drawGeometric(ctx, f.x, f.y, 5 * scale, '#67e8f9', alpha, time)
      break
    case 'story':
      drawConstellation(ctx, f.x, f.y, 5 * scale, '#ffffff', alpha, time)
      break
    case 'question':
      drawPulse(ctx, f.x, f.y, 4 * scale, '#c4b5fd', alpha, time)
      break
  }
}
