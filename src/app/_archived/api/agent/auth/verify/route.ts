...
export async function POST(request: Request) {
  try {
    ...
    const challengePayload = JSON.parse(Buffer.from(challenge, 'base64').toString())
    ...
    const valid = verifyPayload(agent.public_key, message, signature)
    if (!valid) {
