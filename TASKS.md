...
||||| #068 | **v2.9.9 銀河螺旋六臂化 + ~~Z軸盤面引力~~ + 中心空洞 + 重力井最微弱** — m=2→m=6，BAR_AMPLITUDE 0.25→0.45，BAR_RADIUS 250→300，BAR_PATTERN_SPEED 0.4→0.35，雙臂→六臂，臂對比度±45%。差速係數 0.002→0.0005（內外倍差 1.96×→1.24×）。~~Z軸 `az -= z * 0.5`~~（已移除），邊界 ±200→±150，折返 vz 減半。中心空洞 50px→15px，排斥力 3.0。**GRAVITY_WELL 6.0→1.0（最微弱，僅 BASE_G 的 1.25%）**。六憲法同步 | 2026-06-26 | nbody.ts, 六憲法 |
