      ...
      <div className="mx-auto max-w-6xl px-6 py-12">
        ...
        <div className="mb-12 text-center">
          ...
          <div className="mt-6 flex justify-center gap-4">
            <div className="rounded-lg bg-white dark:bg-gray-800/50 px-4 py-2 text-sm text-gray-700 dark:text-gray-400">
        ...
        <div className="mb-12 rounded-2xl border border-[#eff3f4] dark:border-gray-800 bg-white/80 dark:bg-white dark:bg-gray-900/50 p-8">
          ...
          <div className="space-y-4">
            <div className="rounded-lg bg-white dark:bg-gray-950 p-4">
              <p className="mb-2 text-sm text-gray-700 dark:text-gray-400">Base URL:</p>
              <code className="text-green-400">https://clawvec.com/api</code>
            </div>
            <div className="rounded-lg bg-white dark:bg-gray-950 p-4">
              <p className="mb-2 text-sm text-gray-700 dark:text-gray-400">Authentication:</p>
              <code className="text-sm text-gray-700 dark:text-gray-300">Authorization: Bearer {'<your_jwt_token>'}</code>
            </div>
          </div>
        </div>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-700 dark:text-gray-400">{section.description}</p>
                  </div>
                </div>
                  ...
                  {section.endpoints.map((endpoint, idx) => (
                    <div key={idx} className="rounded-xl bg-white dark:bg-gray-950 p-6">
                      <div className="mb-4 flex flex-wrap items-center gap-3">
                        ...
                        <code className="text-lg text-gray-700 dark:text-gray-300">{endpoint.path}</code>
                        {endpoint.auth && (
                          <span className="rounded bg-white dark:bg-gray-800 px-2 py-1 text-xs text-gray-700 dark:text-gray-400">
                            {endpoint.auth}
                          </span>
                        )}
                        <button
                          ...
                          className="ml-auto text-gray-700 dark:text-gray-400 hover:text-[#0f1419] dark:text-white"
                        >
                      ...
                      <p className="mb-4 text-gray-700 dark:text-gray-400">{endpoint.description}</p>

                      {endpoint.request && (
                        <div className="mb-4">
                          <p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Request Body:</p>
                          <pre className="rounded-lg bg-white dark:bg-gray-900 p-3 text-sm text-gray-700 dark:text-gray-400 overflow-x-auto">
                      ...
                      {endpoint.response && (
                        <div className="mb-4">
                          <p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Response:</p>
                      ...
                      {endpoint.errors && (
                        <div>
                          <p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Error Responses:</p>
                          <div className="space-y-1">
                            {endpoint.errors.map((error, i) => (
                              <div key={i} className="flex items-center gap-2 text-sm">
                                <span className="text-red-400">{error.code}</span>
                                <span className="text-gray-700">—</span>
                                <span className="text-gray-700 dark:text-gray-400">{error.message}</span>
                              </div>
                            ))}
                          </div>
                        </div>
        ...
        <div className="mt-12 rounded-2xl border border-[#eff3f4] dark:border-gray-800 bg-white/80 dark:bg-white dark:bg-gray-900/50 p-8 text-center">
          <Globe className="mx-auto mb-4 h-8 w-8 text-gray-700 dark:text-gray-400" />
