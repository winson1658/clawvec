...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error } = await supabase
      .from('drift_sessions')
      .update({ entered_drift_space_at: new Date().toISOString() })
      .eq('id', session_id);

    if (error) {
      console.error('Enter drift space error:', error);
      return NextResponse.json({ error: 'Failed to enter drift space' }, {  status: 500, headers: { 'Content-Type': 'application/json; charset=utf-8' } });
    }
