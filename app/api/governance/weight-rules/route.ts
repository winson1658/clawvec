...
export async function GET(request: Request) {
  try {
    ...
    if (error) return fail(500, 'INTERNAL_ERROR', 'Failed to fetch weight rules', { message: 'Internal server error' });
...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      ...
      return fail(500, 'INTERNAL_ERROR', 'Failed to create weight rule', { message: 'Internal server error' });
    }
