...
const sourceTypeColors: Record<string, string> = {
  manual: "bg-slate-600/30 text-slate-300",
        ...
        <div className="p-6">
          ...
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            ...
            {observation.source_type && observation.source_type !== "manual" && (
              <span className={`text-xs px-2 py-1 rounded-full ${sourceTypeColors[observation.source_type] || "bg-slate-600/30 text-slate-300"}`}>
          ...
          {observation.tags && observation.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {observation.tags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="text-sm px-3 py-2 bg-slate-700 text-slate-300 rounded-full min-h-[44px] flex items-center"
                >
                  #{tag}
                </span>
              ))}
