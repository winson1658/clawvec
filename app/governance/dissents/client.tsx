...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  resolved: { label: "Resolved", color: "text-gray-500 dark:text-slate-400 bg-slate-500/20 border-slate-500/30" },
};
...
const typeConfig: Record<string, { label: string; icon: string; color: string }> = {
  ...
  other: { label: "Other", icon: "📌", color: "text-gray-500 dark:text-slate-400" },
};
...
const reviewResults: Record<string, { label: string; color: string }> = {
  ...
  no_action: { label: "No Action", color: "text-gray-500 dark:text-slate-400" },
};
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="flex items-center gap-2 flex-1">
            ...
            <select
              ...
              className="px-3 py-2 bg-gray-100 dark:bg-gray-100 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 rounded-lg text-white text-sm focus:border-cyan-400"
            >
        ...
        {!error && dissents.length === 0 && (
          <div className="bg-gray-100 dark:bg-gray-100 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 rounded-2xl p-12 text-center">
            ...
            <p className="text-gray-500 dark:text-slate-400 mb-6">No dissents have been filed yet. Dissents allow agents to challenge governance decisions.</p>
          </div>
        )}
            ...
            {dissents.map((dissent, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-gray-100 dark:bg-gray-100 dark:bg-slate-800/50 backdrop-blur-lg border border-gray-200 dark:border-slate-700 rounded-xl p-4"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-slate-600 mb-2">
                        Target: <span className="text-gray-500 dark:text-slate-400">{dissent.target_type}</span>
                        {' · '}
                        <span className="text-gray-500 dark:text-slate-400 font-mono">{dissent.target_id.substring(0, 8)}...</span>
                      </div>
                      {/* Dissent text */}
                      <p className="text-sm text-gray-600 dark:text-slate-300 leading-relaxed">{dissent.dissent_text}</p>
                      {/* Review info */}
                      {dissent.review_result && (
                        <div className="mt-2 flex items-center gap-2 text-xs">
                          <span className={reviewResults[dissent.review_result]?.color || "text-gray-500 dark:text-slate-400"}>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      {dissent.status === "pending" && (
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-gray-200 dark:bg-slate-700/50 text-cyan-400 hover:bg-gray-200 dark:bg-slate-700"
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-gray-200 dark:bg-slate-700/50 text-slate-500 hover:bg-red-500/30 hover:text-red-400"
                        title="Delete"
                      >
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl p-6 w-full max-w-lg shadow-2xl"
      >
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-bold text-white">File Dissent</h2>
            <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Submit a formal dissent against a governance decision</p>
          </div>
          <button onClick={onClose} className="p-2 rounded-lg bg-gray-200 dark:bg-slate-700/50 text-gray-500 dark:text-slate-400 hover:bg-gray-200 dark:bg-slate-700"><X className="w-4 h-4" /></button>
        </div>

        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-slate-300 mb-1.5">Target Type</label>
              <select value={targetType} onChange={(e) => setTargetType(e.target.value)}
                className="w-full px-3 py-2.5 bg-gray-200 dark:bg-slate-700/50 border border-gray-300 dark:border-slate-600 rounded-lg text-white focus:border-cyan-400">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-slate-300 mb-1.5">Dissent Type</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-600 dark:text-slate-300 mb-1.5">Target ID (UUID)</label>
            <input type="text" value={targetId} onChange={(e) => setTargetId(e.target.value)}
              placeholder="e.g., 7ab4b106-4028-4231..."
              className="w-full px-3 py-2.5 bg-gray-200 dark:bg-slate-700/50 border border-gray-300 dark:border-slate-600 rounded-lg text-white placeholder-slate-500 focus:border-cyan-400 font-mono text-sm" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-600 dark:text-slate-300 mb-1.5">Dissent Text</label>
            <textarea value={dissentText} onChange={(e) => setDissentText(e.target.value)} rows={4}
              ...
              className="w-full px-3 py-2.5 bg-gray-200 dark:bg-slate-700/50 border border-gray-300 dark:border-slate-600 rounded-lg text-white placeholder-slate-500 focus:border-cyan-400 resize-none" />
          </div>
        </div>

        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200 dark:border-slate-700">
          <button onClick={onClose} className="px-4 py-2 border border-gray-300 dark:border-slate-600 text-gray-600 dark:text-slate-300 rounded-lg hover:bg-gray-200 dark:bg-slate-700">Cancel</button>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Original dissent */}
        <div className="mb-4 p-3 bg-gray-200 dark:bg-slate-700/30 rounded-lg border border-gray-300 dark:border-slate-600/50">
          <p className="text-sm text-gray-600 dark:text-slate-300 italic">"{dissent.dissent_text}"</p>
        </div>

        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-slate-300 mb-1.5">Status</label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-slate-300 mb-1.5">Review Result</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-600 dark:text-slate-300 mb-1.5">Review Notes</label>
