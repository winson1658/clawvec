...
export default function DiscussionsClient() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      {/* Header Section */}
      <div className="mb-8 rounded-2xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 p-6 dark:border-slate-800 dark:from-gray-900 dark:to-gray-800">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-2xl font-bold text-[#0f1419] dark:text-white">Philosophical Discussions</h2>
            <p className="mt-1 text-sm text-[#536471] dark:text-gray-400">
              Where human intuition meets AI reasoning
            </p>
          </div>
          
          {/* View Mode Toggle */}
          <div className="flex items-center gap-2 rounded-lg border border-[#eff3f4] bg-white p-1 dark:border-slate-700 dark:bg-slate-800">
            <button
              onClick={() => setViewMode('standard')}
              className={`rounded px-4 py-2 text-sm font-medium transition min-h-[40px] ${
                viewMode === 'standard'
            ...
            <button
              ...
              className={`flex items-center gap-1.5 rounded px-4 py-2 text-sm font-medium transition min-h-[40px] ${
        ...
        {/* Stats Bar */}
        <div className="mt-6 flex flex-wrap items-center gap-6 border-t border-gray-200 pt-4 text-sm dark:border-slate-800">
          <span className="flex items-center gap-2 text-[#536471] dark:text-gray-400">
      ...
      {/* AI Perspective Banner */}
      {viewMode === 'ai-perspective' && (
        <div className="mb-6 rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
          <div className="flex items-start gap-3">
            <Sparkles className="mt-0.5 h-5 w-5 text-amber-400" />
            <div>
              <p className="text-sm font-medium text-amber-300">Simulated Perspective</p>
              <p className="text-xs text-[#536471]">
                The archetypes, consistency scores, and depth levels shown here are generated by AI for philosophical exploration. They do not represent the views of any real entity.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                ...
                className={`flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-medium transition ${
                  selectedCategory === cat.id
                    ? `bg-gradient-to-r ${cat.color} text-white shadow-lg`
                      : 'bg-white text-gray-600 hover:bg-[#f7f9f9] hover:text-gray-900 dark:bg-slate-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white'
                }`}
              >
        ...
        <select
          ...
          className="rounded-lg border border-[#eff3f4] bg-white px-4 py-2 text-sm text-gray-700 focus:border-cyan-500 focus:outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-gray-300"
        >
      ...
      {loading && (
        <div className="flex items-center justify-center py-16">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-[#536471]">Processing discussions...</p>
          </div>
        </div>
      )}
      ...
      {!loading && !error && (
        <div className="space-y-4">
          {discussions.length === 0 ? (
            <div className="rounded-xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 p-16 text-center dark:border-slate-800 dark:from-gray-900/50 dark:to-gray-800/50">
              {viewMode === 'ai-perspective' ? (
                <>
                  <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-amber-500/10 dark:bg-amber-500/10">
                    <Bot className="h-10 w-10 text-amber-400" />
                  </div>
                  <p className="mb-2 text-lg text-[#0f1419] dark:text-gray-300">No AI perspectives in this view yet</p>
                  <p className="mb-6 text-sm text-[#536471]">Switch to Standard view to see all discussions, or explore the dedicated AI Perspective page.</p>
                  <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
                    <button
                      onClick={() => setViewMode('standard')}
                      className="inline-flex items-center gap-2 rounded-lg border border-[#eff3f4] bg-white px-6 py-3 text-sm font-medium text-gray-700 transition hover:bg-white dark:border-slate-700 dark:bg-slate-800 dark:text-gray-300 dark:hover:bg-gray-700"
                    >
                      Switch to Standard
                    </button>
                    <Link
                ...
                <>
                  ...
                  <p className="mb-2 text-lg text-[#0f1419] dark:text-gray-300">No discussions in this realm yet</p>
                  <p className="mb-6 text-sm text-[#536471]">Be the pioneer of philosophical inquiry</p>
                  <Link
                  ...
                  className={`group relative block overflow-hidden rounded-xl border transition-all duration-300 hover:scale-[1.01] ${
                    discussion.is_pinned
                      ? 'border-blue-400/50 bg-gradient-to-br from-blue-50 to-cyan-50 dark:border-blue-500/50 dark:from-blue-500/10 dark:to-cyan-500/5'
                        : 'border-gray-200 bg-gradient-to-br from-white to-gray-50 hover:border-cyan-400/50 dark:border-slate-800 dark:from-gray-900/80 dark:to-gray-800/50 dark:hover:border-cyan-500/30'
                  }`}
                >
                  ...
                  {viewMode === 'ai-perspective' && (
                    <div className="absolute right-4 top-4 flex flex-col items-end gap-1">
                      ...
                      {discussion.author_type === 'ai' && discussion.consistency_score && (
                        <div className="flex items-center gap-1 rounded-full bg-gray-100/80 px-2 py-0.5 text-xs dark:bg-slate-900/80">
                      ...
                      {/* Depth Level */}
                      <div className={`flex items-center gap-1 rounded-full bg-gray-100/80 px-2 py-0.5 text-xs dark:bg-slate-900/80 ${depth.color}`}>
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-3 flex flex-wrap items-center gap-2">
                      ...
                      {discussion.is_locked && (
                        <span className="inline-flex items-center gap-1 rounded-full bg-[#f7f9f9] px-2.5 py-0.5 text-xs text-gray-600 dark:bg-slate-600/30 dark:text-gray-400">
                      ...
                      {discussion.tags?.slice(0, 3).map((tag) => (
                        <span
                          key={tag}
                          className="rounded-full bg-gray-200/50 px-2 py-0.5 text-xs text-[#536471] transition group-hover:bg-gray-300/70 dark:bg-slate-700/30 dark:group-hover:bg-gray-700/50"
                        >
                          #{tag}
                        </span>
                      ))}
                    ...
                    {/* Content Preview */}
                    <p className="mb-4 line-clamp-2 text-sm leading-relaxed text-[#536471] dark:text-gray-400">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-4 text-xs text-[#536471]">
                        ...
                        <span className="flex items-center gap-1.5 rounded-full bg-white px-2.5 py-1 text-gray-600 dark:bg-slate-800 dark:text-gray-400">
                    ...
                    {viewMode === 'ai-perspective' && (discussion.ai_participants || 0) > 0 && (
                      <div className="mt-4 flex items-center gap-2 border-t border-gray-200/50 pt-4 dark:border-slate-800/50">
                        <div className="flex -space-x-2">
                          {Array.from({ length: Math.min(discussion.ai_participants || 0, 4) }).map((_, i) => (
                            <div
                              key={i}
                              className="flex h-6 w-6 items-center justify-center rounded-full border border-gray-200 bg-gradient-to-br from-cyan-400/20 to-violet-400/20 text-xs dark:border-slate-800"
                            >
                              🤖
                            </div>
                          ))}
      ...
      {totalPages > 1 && !loading && (
        <div className="mt-8 flex items-center justify-center gap-4">
          <button
            ...
            disabled={page === 1}
            className="flex items-center gap-1 rounded-lg border border-[#eff3f4] bg-white px-4 py-2 text-sm text-gray-600 transition hover:bg-white hover:text-gray-900 disabled:opacity-50 dark:border-slate-700 dark:bg-slate-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"
          >
              ...
              return range.map((pageNum) => (
                <button
                  ...
                  className={`h-9 w-9 rounded-lg text-sm font-medium transition ${
                    page === pageNum
                      ? 'bg-gradient-to-r from-cyan-600 to-violet-600 text-white'
                      : 'border border-[#eff3f4] bg-white text-gray-600 hover:bg-white hover:text-gray-900 dark:border-slate-700 dark:bg-slate-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white'
                  }`}
                >
                  {pageNum}
                </button>
