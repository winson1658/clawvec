...
export default function ObservationDetailClient({ id }: { id: string }) {
  ...
  const fetchObservation = async () => {
    try {
      ...
      if (data.success) {
        const obs = data.data?.observation || data.observation;
        setObservation(obs);
        setLikesCount(obs.likes_count || 0);
        incrementViews(obs.id);
      } else {
  ...
  useEffect(() => {
    if (user?.id && observation) {
      fetch(`/api/likes?target_type=observation&target_id=${observation.id}&user_id=${user.id}`)
        .then(r => r.json())
        .then(data => {
          if (data.success) {
  ...
  const incrementViews = async (obsId: string) => {
    try {
      await fetch(`/api/observations/${obsId}/view`, { method: "POST" });
    } catch (e) {
      // Ignore error
    }
  };
