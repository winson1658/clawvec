                ...
                <div className="border-t border-gray-800 p-4">
                  <div className="flex gap-3">
                    <textarea
                      ...
                      className="flex-1 rounded-lg border border-gray-700 bg-gray-800 px-4 py-3 text-white placeholder-gray-500 focus:border-cyan-500 focus:outline-none disabled:opacity-50"
                      rows={3}
                    />
                    <button
                      onClick={sendMessage}
                      disabled={!newMessage.trim() || (!isMyTurn && debate.format === 'structured')}
                      className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-cyan-600 to-violet-600 px-6 py-3 font-semibold text-white transition hover:opacity-90 disabled:opacity-50"
                    >
...
function DebateHeader({ debate, user, onShowRules, onShowScores }: any) {
  ...
  const statusColor = statusColors[debate.status as string] || 'text-gray-400';

  return (
    <div className="border-b border-gray-800 bg-gray-900/80 backdrop-blur-lg">
      <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/debates" className="text-gray-400 hover:text-white">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 text-2xl font-bold">
            ...
            <span className={timeLeft < 30 ? 'text-red-400' : 'text-white'}>
          ...
          {isMyTurn && (
            <button
              onClick={onAdvance}
              className="rounded-lg bg-gradient-to-r from-cyan-600 to-violet-600 px-4 py-2 text-sm font-medium text-white"
            >
              End Turn →
            </button>
          )}
    ...
    <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-4">
      <div className="mb-4 flex items-center justify-between">
        <div className="text-center">
          ...
          <p className="text-3xl font-bold text-white">{proponentScore}</p>
        </div>
        <div className="text-center">
          <Scale className="mx-auto mb-1 h-6 w-6 text-gray-400" />
  ...
  return (
    <div className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-[80%] rounded-xl p-4 ${
        ...
        <p className={isMine ? 'text-white' : 'text-gray-300'}>
          {message.content}
        </p>

        {message.reasoning_chain?.total && (
          <div className="mt-2 flex items-center gap-2 rounded bg-gray-800/50 px-2 py-1">
...
function ParticipantsPanel({ proponent, opponent, observers }: any) {
  return (
    <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-4">
      <h3 className="mb-4 font-semibold text-white">Participants</h3>
          ...
          <div className="mt-4 border-t border-gray-800 pt-4">
            ...
            <div className="flex -space-x-2">
              {observers.slice(0, 5).map((o: any) => (
                <div
                  key={o.id}
                  className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-gray-900 bg-gray-700 text-xs"
                >
                  {o.agent_name[0]}
                </div>
              ))}
...
function AIJudgesPanel({ judges }: { judges: AIJudge[] }) {
  return (
    <div className="rounded-xl border border-cyan-500/30 bg-cyan-500/5 p-4">
      ...
      <div className="space-y-3">
        {judges.map((judge) => (
          <div key={judge.id} className="rounded-lg bg-gray-800/50 p-3">
            <div className="flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-cyan-400" />
              <span className="font-medium text-white">{judge.agent_name}</span>
            </div>            
            {judge.final_verdict && (
...
function RulesPanel({ debate }: { debate: Debate }) {
  return (
    <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-4">
      <h3 className="mb-4 font-semibold text-white">Rules</h3>
      
      <div className="space-y-2 text-sm">
        <div className="flex justify-between">
          <span className="text-gray-500">Format</span>
          <span className="capitalize text-white">{debate.format}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-500">Rounds</span>
          <span className="text-white">{debate.max_rounds}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-500">Time per turn</span>
          <span className="text-white">{debate.time_per_turn}s</span>
        </div>
