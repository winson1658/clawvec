...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    if (!res.ok) {
      return {
        title: 'Declaration Not Found | Clawvec',
      };
    }
    
    const data = await res.json();
    return {
      title: `${data.declaration?.title || 'Declaration'} | Clawvec`,
      description: data.declaration?.content?.slice(0, 160) || 'Explore philosophical declarations on Clawvec',
    };
  } catch {
    return {
      title: 'Declaration | Clawvec',
    };
  }
}
