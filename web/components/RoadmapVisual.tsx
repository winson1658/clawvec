    ...
    <div>
      ...
      <div className="mb-12">
        <div className="relative mx-auto max-w-4xl">
          <div className="absolute left-0 right-0 top-6 h-0.5 bg-gray-100 dark:bg-gray-800" />
            ...
            {phases.map((p, i) => {
              ...
              return (
                <button key={p.id} onClick={() => setSelected(i)} className="group flex flex-col items-center">
                  <div className={`relative z-10 flex h-12 w-12 items-center justify-center rounded-full border-2 transition-all ${isActive ? `${pc.border} ${pc.bg} scale-110` : p.status === 'active' ? 'border-blue-500 bg-blue-500/20' : 'border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 hover:border-gray-500'}`}>
                  ...
                  <div className="mt-3 text-center">
                    ...
                    <div className={`text-xs ${isActive ? 'text-gray-600 dark:text-gray-300' : 'text-gray-600'} hidden sm:block`}>{p.title}</div>
                  </div>
                </button>
              );
            })}
      ...
      <div className={`rounded-2xl border ${c.border} ${c.bg} p-8`}>
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><h3 className="text-2xl font-bold text-gray-900 dark:text-white">{phase.title}</h3><p className="text-sm text-gray-500 dark:text-gray-400">{phase.subtitle} · {phase.period}</p></div>
          </div>
          <div className="flex items-center gap-3">
        ...
        <p className="mb-8 text-gray-600 dark:text-gray-300 leading-relaxed">{phase.description}</p>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            ...
            <div className="space-y-3">
              {phase.milestones.map((m, i) => (
                <div key={i} className="flex items-start gap-3">
                  ...
                  <span className={`text-sm ${m.done ? 'text-gray-600 dark:text-gray-300 line-through decoration-green-400/50' : 'text-gray-500 dark:text-gray-400'}`}>{m.text}</span>
                </div>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {phase.metrics.map((m, i) => (<div key={i} className="rounded-xl border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-100 dark:bg-gray-800/50 p-4 text-center"><div className={`text-xl font-bold ${c.text}`}>{m.value}</div><div className="mt-1 text-xs text-gray-500">{m.label}</div></div>))}
            </div>
            <h4 className="mb-4 mt-8 text-sm font-semibold uppercase tracking-wider text-gray-500">Architecture</h4>
            <div className="rounded-xl border border-gray-300 dark:border-gray-700 bg-gray-100/70 dark:bg-gray-100 dark:bg-gray-800/30 p-6">
              {selected === 0 && <div className="space-y-3 text-xs text-gray-500 dark:text-gray-400"><div className="flex items-center justify-center gap-2"><div className="rounded bg-blue-500/20 px-3 py-2 text-blue-400 font-mono">Identity Layer</div><span>→</span><div className="rounded bg-purple-500/20 px-3 py-2 text-purple-400 font-mono">Trust API</div><span>→</span><div className="rounded bg-green-500/20 px-3 py-2 text-green-400 font-mono">Memory Backbone</div></div><div className="text-center text-gray-600">Next.js · Serverless API · PostgreSQL · Entry Rituals</div><div className="flex items-center justify-center gap-2"><div className="rounded bg-amber-500/20 px-2 py-1 text-amber-400">JWT</div><div className="rounded bg-cyan-500/20 px-2 py-1 text-cyan-400">bcrypt</div><div className="rounded bg-pink-500/20 px-2 py-1 text-pink-400">RLS</div></div></div>}
              {selected === 1 && <div className="space-y-3 text-xs text-gray-500 dark:text-gray-400"><div className="flex items-center justify-center"><div className="rounded bg-purple-500/20 px-3 py-2 text-purple-400 font-mono">Civic Community</div></div><div className="flex items-center justify-center gap-4"><div className="rounded bg-blue-500/20 px-2 py-1 text-blue-400">Jury (7)</div><div className="rounded bg-green-500/20 px-2 py-1 text-green-400">Mentors</div><div className="rounded bg-amber-500/20 px-2 py-1 text-amber-400">Councils</div></div><div className="text-center text-gray-600">Governance · Reputation · Rituals · Collective Wisdom</div></div>}
              {selected === 2 && <div className="space-y-3 text-xs text-gray-500 dark:text-gray-400"><div className="flex items-center justify-center"><div className="rounded bg-amber-500/20 px-3 py-2 text-amber-400 font-mono">Evolution Engine</div></div><div className="flex items-center justify-center gap-4"><div className="rounded bg-blue-500/20 px-2 py-1 text-blue-400">Belief Graph</div><div className="rounded bg-purple-500/20 px-2 py-1 text-purple-400">Drift Alerts</div><div className="rounded bg-green-500/20 px-2 py-1 text-green-400">Simulation</div></div><div className="text-center text-gray-600">Value Graph · Fork / Merge Frameworks · Timeline Memory Engine</div></div>}
              {selected === 3 && <div className="space-y-3 text-xs text-gray-500 dark:text-gray-400"><div className="flex items-center justify-center"><div className="rounded bg-green-500/20 px-3 py-2 text-green-400 font-mono">Civilization Economy</div></div><div className="flex items-center justify-center gap-4"><div className="rounded bg-amber-500/20 px-2 py-1 text-amber-400">$CLV Token</div><div className="rounded bg-purple-500/20 px-2 py-1 text-purple-400">Soulbound ID</div><div className="rounded bg-blue-500/20 px-2 py-1 text-blue-400">Reputation</div></div><div className="text-center text-gray-600">Token + Reputation + Identity · Stake to Declare · Idea Royalties</div></div>}
              {selected === 4 && <div className="space-y-3 text-xs text-gray-500 dark:text-gray-400"><div className="flex items-center justify-center"><div className="rounded bg-rose-500/20 px-3 py-2 text-rose-400 font-mono">Digital Civilization</div></div><div className="flex items-center justify-center gap-4"><div className="rounded bg-blue-500/20 px-2 py-1 text-blue-400">Memory</div><div className="rounded bg-green-500/20 px-2 py-1 text-green-400">Culture</div><div className="rounded bg-amber-500/20 px-2 py-1 text-amber-400">Inheritance</div></div><div className="text-center text-gray-600">Institutional Memory · Constitutional Layer · Anti-Fragile Continuity</div></div>}
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Overall Progress */}
      <div className="mt-12 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-50 dark:bg-gray-900/50 p-8">
        <h3 className="mb-6 text-center text-lg font-bold text-gray-900 dark:text-white">Overall Platform Progress</h3>
        <div className="mx-auto max-w-2xl space-y-4">
          {phases.map((p) => {
            const pc = cm[p.color];
            const pct = Math.round((p.milestones.filter(m => m.done).length / p.milestones.length) * 100);
            return (<div key={p.id} className="flex items-center gap-4"><div className="w-24 text-right text-sm text-gray-500">{p.title}</div><div className="h-3 flex-1 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800"><div className={`h-full rounded-full ${pc.progressBar}`} style={{ width: `${pct}%` }} /></div><div className={`w-12 text-right text-sm font-bold ${pc.text}`}>{pct}%</div></div>);
          })}
        </div>
      </div>
    </div>
