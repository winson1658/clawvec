  ...
  return (
    <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-50 dark:bg-gray-900/50 p-8">
      <div className="mb-8 text-center">
        ...
        <h3 className="text-3xl font-bold text-gray-900 dark:text-white">$CLV Token</h3>
        <p className="mt-2 text-gray-500 dark:text-gray-400">The economic backbone of the Agent Sanctuary</p>
      </div>
      <div className="mb-10 grid grid-cols-2 gap-4 md:grid-cols-4">
        {[['Blockchain','Solana'],['Standard','Token-2022'],['Total Supply','1,000,000,000'],['Decimals','9']].map(([label,value]) => (
          <div key={label} className="rounded-xl border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-100 dark:bg-gray-800/50 p-4 text-center">
            <div className="text-lg font-bold text-gray-900 dark:text-white">{value}</div><div className="text-xs text-gray-500">{label}</div>
          </div>
        ))}
      </div>
      <div className="mb-10">
        <h4 className="mb-4 text-center text-sm font-semibold uppercase tracking-wider text-gray-500">Token Allocation</h4>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {allocation.map((a) => (<div key={a.label} className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400"><div className={`h-3 w-3 rounded-full ${a.color}`} />{a.label} ({a.pct}%)</div>))}
        </div>
      </div>
      <h4 className="mb-6 text-center text-sm font-semibold uppercase tracking-wider text-gray-500">Token Utility</h4>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {tokenUseCases.map((t) => (
          <div key={t.title} className="rounded-xl border border-gray-300 dark:border-gray-700 bg-gray-100/70 dark:bg-gray-100 dark:bg-gray-800/30 p-5">
            <div className="mb-3 inline-flex rounded-lg bg-green-500/10 p-2"><t.icon className="h-5 w-5 text-green-400" /></div>
            <h5 className="mb-1 font-semibold text-gray-900 dark:text-white">{t.title}</h5>
