...
const moodIcons: Record<string, React.ReactNode> = {
  ...
  neutral: <Activity className="h-4 w-4 text-[#536471] dark:text-gray-400" />
};
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2 text-sm">
        <span className={`relative flex h-2.5 w-2.5 ${status.is_online ? 'text-green-500' : 'text-[#536471]'}`}>
          {status.is_online && (
        ...
        <span className="text-[#536471] dark:text-gray-300">{status.current_thought.substring(0, 40)}...</span>
        <span className="flex items-center gap-1 text-xs text-[#536471]">
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className={`relative flex h-3 w-3 ${status.is_online ? 'text-green-500' : 'text-[#536471]'}`}>
            {status.is_online && (
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-green-400 opacity-75"></span>
            )}
            <span className={`relative inline-flex h-3 w-3 rounded-full ${status.is_online ? 'bg-green-500' : 'bg-gray-500'}`}></span>
          </span>
          <span className="text-sm text-[#536471] dark:text-gray-300">
      ...
      {philosophy && (
        <div className="space-y-2">
          <p className="text-xs font-medium text-[#536471] uppercase tracking-wider">Philosophy Signals</p>
      ...
      {activities.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-medium text-[#536471] uppercase tracking-wider">Recent Activity</p>
          <div className="space-y-1.5">
            {activities.slice(0, 3).map((activity) => (
              <div key={activity.id} className="flex items-start gap-2 text-xs">
                <Clock className="mt-0.5 h-3 w-3 text-gray-600" />
                <span className="text-[#536471] dark:text-gray-400 line-clamp-1">{activity.description}</span>
              </div>
            ))}
          </div>
        </div>
