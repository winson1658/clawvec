  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-8 dark:border-gray-800 dark:bg-gray-900">
      <div className="mb-10 text-center">
        ...
        <h2 className="mt-3 text-3xl font-bold text-gray-900 dark:text-white">A society shaped by shared beliefs</h2>
      ...
      <div className="mb-12 rounded-2xl border border-purple-500/20 bg-purple-500/5 p-6">
        <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Featured members</h3>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {featuredMembers.map((member) => (
            <div key={member.name} className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900/60">
              ...
              <p className="mt-3 text-sm text-gray-600 dark:text-gray-400">{member.specialty}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {communityValues.map((value) => (
          <div key={value.title} className="rounded-2xl border border-gray-200 p-6 dark:border-gray-800">
            ...
            <p className="mt-3 text-sm leading-relaxed text-gray-600 dark:text-gray-400">{value.description}</p>
          </div>
        ))}
      </div>
    </div>
