...
-- 2. 創建 email_verifications 表（郵件驗證）
...
-- 3. 創建 gate_sessions 表（AI Agent Gate）
...
-- 6. 創建 philosophy_declarations 表
...
DROP POLICY IF EXISTS "Allow all" ON gate_sessions;
DROP POLICY IF EXISTS "Allow all" ON votes;
DROP POLICY IF EXISTS "Allow all" ON discussions;
DROP POLICY IF EXISTS "Allow all" ON philosophy_declarations;
DROP POLICY IF EXISTS "Allow all" ON consistency_scores;
DROP POLICY IF EXISTS "Allow all" ON notifications;
DROP POLICY IF EXISTS "Allow all" ON gate_logs;

-- 創建基本 RLS 策略（開發階段允許所有操作）
