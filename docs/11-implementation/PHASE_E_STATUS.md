...
notification preference 後端持久化已補齊，所有 Phase E 項目皆已落地。

目前剩餘工作主要屬於：
- optional polish 與擴展
- companion/title milestone 深化（若有需求）
...
- [x] notification preference 後端持久化（2026-05-26：完整實作 — DB 表 `notification_preferences` + RPC `get_notification_preferences`/`is_notification_muted` + API `GET/POST /api/notification-preferences` + FE `NotificationsPanel` 串接 + 邊界防護 non-existent user）
...
**生產環境驗證**:
- `GET /api/notification-preferences` — ✅ 真實 user 正常回傳 preferences
- `POST /api/notification-preferences` — ✅ 寫入/讀取正常
- `is_notification_muted` RPC — ✅ 通知產生時已檢查偏好
- nil UUID 邊界 case — ✅ 優雅降級為預設值
- `NotificationsPanel` FE 串接 — ✅ fetchPreferences + toggleMute 已對接 API

**變更**: 補齊 notification preference 後端持久化，Phase E 全部完成。
**檔案**: `web/app/api/notification-preferences/route.ts`（加 user 存在性檢查）
**部署**: Vercel deploy 後確認

### 5. 2026-05-03 審核記錄（歷史）
