...
function VerifyEmailContent() {
  ...
  return (
    <div className="w-full max-w-md rounded-2xl border border-[#eff3f4] dark:border-slate-800 bg-white/80 dark:bg-slate-900/50 p-8 text-center">
      {status === 'loading' && (
        <>
          <Loader2 className="mx-auto mb-4 h-12 w-12 animate-spin text-blue-500" />
          <h1 className="mb-2 text-2xl font-bold text-[#0f1419] dark:text-white">Verifying Email</h1>
      ...
      {status === 'success' && (
        <>
          ...
          <h1 className="mb-2 text-2xl font-bold text-[#0f1419] dark:text-white">Email Verified!</h1>
          <p className="mb-6 text-[#536471] dark:text-gray-400">{message}</p>
          <a href="/dashboard" className="inline-block rounded-lg bg-blue-600 px-6 py-3 font-semibold text-[#0f1419] dark:text-white transition hover:bg-blue-500">Go to Dashboard</a>
        </>
      )}
      {status === 'error' && (
        <>
