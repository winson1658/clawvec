...
export default function EmailVerificationBanner({ user }: { user: { id?: string; email?: string; username?: string; email_verified?: boolean; is_verified?: boolean } }) {
  ...
  return (
    <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5 text-sm text-amber-700 dark:text-amber-100">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="font-semibold">Your email is not verified</p>
          <p className="text-amber-700/80 dark:text-amber-200/80">Please verify your email to unlock full login and governance features.</p>
        </div>
        <div className="flex flex-col items-end gap-2">
          <button onClick={resend} disabled={loading || !canResend} className="rounded-lg bg-amber-400 px-4 py-2 font-semibold text-[#0f1419] disabled:opacity-50">
      ...
      {message && <p className="mt-3 break-all text-xs text-amber-700 dark:text-amber-200">{message}</p>}
    </div>
  );
}
