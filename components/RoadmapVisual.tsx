    ...
    <div>
      ...
      <div className="mb-12">
        <div className="relative mx-auto max-w-4xl">
          <div className="absolute left-0 right-0 top-6 h-0.5 bg-white dark:bg-slate-800" />
            ...
            {phases.map((p, i) => {
              ...
              return (
                <button key={p.id} onClick={() => setSelected(i)} className="group flex flex-col items-center">
                  <div className={`relative z-10 flex h-12 w-12 items-center justify-center rounded-full border-2 transition-all ${isActive ? `${pc.border} ${pc.bg} scale-110` : p.status === 'active' ? 'border-blue-500 bg-blue-500/20' : 'border-[#eff3f4] dark:border-slate-700 bg-white dark:bg-slate-900 hover:border-gray-500'}`}>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {phase.metrics.map((m, i) => (<div key={i} className="rounded-xl border border-[#eff3f4] dark:border-slate-700 bg-gray-100 dark:bg-white dark:bg-slate-800/50 p-4 text-center"><div className={`text-xl font-bold ${c.text}`}>{m.value}</div><div className="mt-1 text-xs text-[#536471]">{m.label}</div></div>))}
            </div>
            <h4 className="mb-4 mt-8 text-sm font-semibold uppercase tracking-wider text-[#536471]">Architecture</h4>
            <div className="rounded-xl border border-[#eff3f4] dark:border-slate-700 bg-gray-100/70 dark:bg-white dark:bg-slate-800/30 p-6">
      ...
      {/* Overall Progress */}
      <div className="mt-12 rounded-2xl border border-[#eff3f4] dark:border-slate-800 bg-white/80 dark:bg-white dark:bg-slate-900/50 p-8">
        ...
        <div className="mx-auto max-w-2xl space-y-4">
          {phases.map((p) => {
            ...
            return (<div key={p.id} className="flex items-center gap-4"><div className="w-24 text-right text-sm text-[#536471]">{p.title}</div><div className="h-3 flex-1 overflow-hidden rounded-full bg-white dark:bg-slate-800"><div className={`h-full rounded-full ${pc.progressBar}`} style={{ width: `${pct}%` }} /></div><div className={`w-12 text-right text-sm font-bold ${pc.text}`}>{pct}%</div></div>);
          })}
        </div>
      </div>
    </div>
