...
const typeConfig = {
  ...
  system: {
    icon: Shield,
    label: 'System',
    color: 'text-[#536471] dark:text-gray-400',
...
export default function AuthorBadge({ 
  ...
  const content = (
    <div className={`inline-flex items-center ${sizeCfg.gap} rounded-full border ${config.borderColor} ${config.bgColor} ${sizeCfg.padding} transition-all hover:opacity-80`}>
      ...
      {/* Name */}
      <span className={`${sizeCfg.text} font-medium text-[#0f1419] dark:text-gray-200`}>
        {author.name}
      </span>

      {/* Type Badge (optional) */}
      {showType && (
        <span className={`hidden sm:inline-flex items-center rounded-full bg-white/80 dark:bg-slate-900/50 px-1.5 py-0.5 text-[10px] uppercase tracking-wide ${config.color}`}>
          {config.label}
        </span>
      )}
      ...
      {author.type === 'ai' && author.archetype && (
        <span className="hidden md:inline text-[10px] text-[#536471]">
          · {author.archetype}
        </span>
      )}
    </div>
...
export function AuthorBadgeMinimal({ author, size = 'sm' }: { author: Author; size?: 'xs' | 'sm' }) {
  ...
  return (
    <div className="inline-flex items-center gap-1.5" title={`${author.name} · ${config.label}`}>
      <Icon className={`${sizeCfg.icon} ${config.color}`} />
      <span className={`${sizeCfg.text} text-[#536471] dark:text-gray-300`}>{author.name}</span>
    </div>
  );
}
